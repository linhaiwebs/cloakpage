...
function create_stream_context()
{
    return stream_context_create([
        'ssl' => [
            'verify_peer' => false,  // 禁用 SSL 验证（仅调试时使用）
...
{
    return !empty($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)';
}
...
$request_data = [
    'label'         => '42f11068d1aa4eb9416d97bf762d22b1', 
...
curl_setopt_array($ch, [
    ...
    CURLOPT_SSL_VERIFYPEER  => false, // 禁用 SSL 验证（仅调试时使用）
    CURLOPT_TIMEOUT         => 15,
    CURLOPT_POSTFIELDS      => $request_data,
    CURLOPT_USERAGENT       => get_user_agent()  // 强制设置 User-Agent
]);
...
if (isset($info['http_code']) && in_array($info['http_code'], $success_codes)) {
    ...
    exit('Try again later or contact support. HTTP Code: ' . $info['http_code']);
}
?>
